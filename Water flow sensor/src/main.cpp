 ...
 */
#define sensorInterrupt 12 //config pin;  This example use Lambda board, if 0 --> connect at digital pin 2
#define sensorPin 2
...
void loop() {
  ...
  if(delTime >= stepTime){ //measureFlow in each stepTime. For example: each 1 second)
    measureFlow();
    if(flowCumVol >= 5){
      cost += (flowCumVol/1000)*10;
      flowCumVol = 0;
      cloud_cost = Firebase.getFloat("/cost/water");
      Firebase.setFloat("/cost/water", cloud_cost+cost);
      Serial.print("Cost: "); Serial.print(cost); Serial.println(" Baht");
